...
//                                final List<MatchResult> eaBuildNumberResults = eaBuildNumberMatcher.results().collect(Collectors.toList());
                                final List<MatchResult> eaBuildNumberResults = new ArrayList<>();
                                while (eaBuildNumberMatcher.find()) {
                                    eaBuildNumberResults.add(eaBuildNumberMatcher.toMatchResult());
                                }
    ...
    @Override public int compareTo(final VersionNumber otherVersionNumber) {
        ...
        if (ret == equal) {
            ...
            } else if (releaseStatus.isPresent() && ReleaseStatus.EA == releaseStatus.get() && build.isPresent() && otherVersionNumber.getReleaseStatus().isPresent() && ReleaseStatus.EA == otherVersionNumber.getReleaseStatus().get() && !otherVersionNumber.getBuild().isPresent()) {
                ret = largerThan;
